...
contract GameAirdropSignedTest is Test {
    GameAirdropSigned public gameAirdrop;
    ...
    function setUp() public {
        ...
        vm.startPrank(owner);
        gameAirdrop = new GameAirdropSigned{value: INITIAL_FUNDING}(
    ...
    function test_RecordRewardETH() public {
        bytes memory signature = _signReward(player1, true);
        vm.startPrank(owner);
        vm.expectEmit(true, true, true, true);
        emit RewardRecorded(player1, ETH_PAYOUT, true);
        gameAirdrop.recordReward(player1, true, block.timestamp + 1 hours, signature);
    ...
    function test_RecordRewardUSDC() public {
        bytes memory signature = _signReward(player1, false);
        vm.startPrank(owner);
        vm.expectEmit(true, true, true, true);
        emit RewardRecorded(player1, USDC_PAYOUT, false);
        gameAirdrop.recordReward(player1, false, block.timestamp + 1 hours, signature);
        (uint256 ethReward, uint256 usdcReward) = gameAirdrop.getRewards(player1);
    ...
    function testFail_RecordRewardWithInvalidSignature() public {
        bytes memory invalidSignature = _signReward(player2, true); // Sign for wrong player
        vm.startPrank(owner);
        gameAirdrop.recordReward(player1, true, block.timestamp + 1 hours, invalidSignature);
        vm.stopPrank();
    }

    function testFail_RecordRewardWithWrongTokenType() public {
        bytes memory signature = _signReward(player1, true); // Sign for ETH
        vm.startPrank(owner);
        gameAirdrop.recordReward(player1, false, block.timestamp + 1 hours, signature); // Try to record USDC reward
        vm.stopPrank();
    }

    function _signReward(address player, bool isETH) internal view returns (bytes memory) {
        bytes32 messageHash = keccak256(abi.encodePacked(player, isETH, block.timestamp + 1 hours, address(gameAirdrop)));
